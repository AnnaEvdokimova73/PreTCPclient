...
void MainWindow::DisplayStat(StatServer stat)
{
    ...
    ui->tb_result->append(QString("Количество принятых байт: %1").arg(stat.incBytes));
    ui->tb_result->append(QString("Количество переданных байт: %1").arg(stat.sendBytes));
    ui->tb_result->append(QString("Количество принятых пакетов: %1").arg(stat.revPck));
    ui->tb_result->append(QString("Количество переданных пакетов: %1").arg(stat.sendPck));
    ui->tb_result->append(QString("Время работы сервера, сек: %1").arg(stat.workTime));
    ui->tb_result->append(QString("Количество подключённых клиентов: %1").arg(stat.clients));
